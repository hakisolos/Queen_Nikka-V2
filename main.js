...
require('./Queen-Nikka.js')
nocache('../Queen-Nikka.js', module => console.log(color('[ CHANGE ]', 'green'), color(`'${module}'`, 'green'), 'Updated'))
require('./main.js')
...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
    ...
    const XeonBotInc = makeWASocket({
     ...
     auth: {
      ...
      getMessage: async (key) => {
            ...
            return {
                conversation: "LONG LIVE THE QUEEN👑!"
            }
        },
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`input your number with country code👑\nFor example: +2349112171078 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            ...
            phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`nput your number with country code👑\nFor example: +2349112171078 : `)))
            phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
            rl.close()
         }
      }
...
try{
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
	        ...
	        console.log(color(`\n${themeemoji} 👑BOT NAME: QUEEN NIKKA V2`,'orange'))
            console.log(color(`${themeemoji} OWNER: HAKI `,'orange'))
            console.log(color(`${themeemoji} GITHUB: maxwellexcel`,'orange'))
...
let xeonName = num
xeonbody = ` YAY🎉 @${xeonName.split("@")[0]}, you have been *promoted* to *admin* 👑`
...
const xeondate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
let xeonName = num
xeonbody = `bummer‼️ @${xeonName.split("@")[0]}, you have been *demoted* from *admin* 😬`
XeonBotInc.sendMessage(anu.id,
 { text: xeonbody,
 contextInfo:{
 mentionedJid:[num],
